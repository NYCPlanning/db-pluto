...
for i in range(len(rpad)):
    if locs['communityDistrict'][i] != 'none':
        upd = "UPDATE pluto_rpad_geo a SET cd = '"+ str(locs['communityDistrict'][i]) +"' WHERE a.gihighhousenumber1::text = '"+ rpad['gihighhousenumber1'][i] +"' AND a.gistreetname1 = '"+ rpad['gistreetname1'][i] +"' AND a.borough = '"+ rpad['borough'][i] +"';"
