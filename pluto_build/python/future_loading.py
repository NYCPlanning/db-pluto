...
def ETL(schema_name, version="latest"):
    ...
    x = requests.post(
        ...
        data=json.dumps(
            {
                "connection": {"build_engine": BUILD_ENGINE},
                "config": {"schema_name": schema_name, "version": version},
            }
        ),
    )
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(
        "https://raw.githubusercontent.com/NYCPlanning/db-pluto/future/pluto_build/data/pluto_input_research.csv",
        index_col=False,
        dtype=str,
    )
    ...
    df = pd.read_csv(
        "https://raw.githubusercontent.com/NYCPlanning/db-pluto/future/pluto_build/output/pluto_corrections.csv",
        index_col=False,
        dtype=str,
    )
    df.columns = [i.lower() for i in df.columns]
    df.to_sql(con=con, name="pluto_corrections", if_exists="replace", index=False)
