...
psql $BUILD_ENGINE -f sql/create_allocated.sql # 48s
...
echo '\nComputing zoning fields \e[32m' #16 min
...
echo '\nFilling in FAR values \e[32m' #4 min
...
echo '\nAdding in geometries that are in the DTM but not in RPAD' #1 min
psql $BUILD_ENGINE -f sql/dtmgeoms.sql
psql $BUILD_ENGINE -f sql/geomclean.sql

echo '\nFlagging tax lots within the FEMA floodplain \e[32m' #9 min
...
echo '\nPopulating PLUTO tags and version fields \e[32m' #7 min
